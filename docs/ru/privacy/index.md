---
title: Конфиденциальность | Dotfiles (Русский)
description: Политика конфиденциальности документации Dotfiles. Совместимо с macOS, Linux и WSL через Dotfiles.
lang: ru-RU
author: Sebastien Rousseau
lastUpdated: true
image: /logo.png
imageAlt: Конфиденциальность page preview
canonical: /ru/privacy/
robots: index, follow
draft: false
noindex: false
sitemap: true
breadcrumbTitle: Конфиденциальность
pageType: legal
schemaType: WebPage
---

# Конфиденциальность

- Минимальный сбор данных.
- Логи используются для надёжности и безопасности.
- Персональные данные не продаются.

Контакт по вопросам конфиденциальности: [info@dotfiles.io](mailto:info@dotfiles.io).
