...
lang: zh-CN
metaTitle: Default aliases - Dotfiles (CN)
...
meta:
  ...
  - name: twitter:description
    content: The Default aliases are aliases that are not specific to any particular command. They are common aliases that are used in many different contexts and are listed here for convenience.
  - name: twitter:title
    content: Default aliases - Dotfiles (CN)
  - name: og:title
    content: Default aliases - Dotfiles (CN)
  - name: og:description
    content: The Default aliases are aliases that are not specific to any particular command. They are common aliases that are used in many different contexts and are listed here for convenience.
...
| ctf      | `echo $(ls -1                        | wc -l)`                                                                                                                                 | Count the number of 文件 in the current 目录. |
...
| now      | `date +"%T"`                         | 显示 the current time.                                                                                                                  |
| op       | `sudo lsof -i -P`                    | 列出 of 打开 ports.                                                                                                                     |
| p        | `pwd`                                | Shortcut for `pwd` which returns working 目录 name.                                                                                |
...
| spd      | `sudo rm -rf /private/var/log/asl/*` | 移除 all log 文件 in /private/var/log/asl.                                                                                           |
| srv      | `python3 -m http.server`             | 开始 a simple HTTP server.                                                                                                             |
| t        | `tail -f`                            | Prints the last 10 lines of a text or log 文件, and then waits for new additions to the 文件 to 打印 it in real time.                  |
| top      | `sudo btop`                          | Allows the user to interactively monitor the system's vital resources or server's processes in real time.                               |
| tree     | `tree --dirsfirst`                   | Display a 目录 tree.                                                                                                               |
| wk       | `date +%V`                           | 显示 the current week number.                                                                                                           |
