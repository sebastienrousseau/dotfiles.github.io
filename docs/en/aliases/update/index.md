---
title: Update Aliases | Dotfiles
description: Explore update aliases in Dotfiles v0.2.495. One-command system and package updates for Homebrew, apt, and dnf on macOS, Linux, and WSL.
lang: en-GB
author: Sebastien Rousseau
lastUpdated: true
image: /logo.png
imageAlt: Update Aliases | Dotfiles preview
canonical: /en/aliases/update/
robots: index, follow
draft: false
noindex: false
sitemap: true
breadcrumbTitle: Update Aliases
pageType: docs
schemaType: WebPage
permalink: /en/aliases/update/
---

# Update Aliases

Short update commands.

This page introduces one alias group and its common command shortcuts.
Read it first to understand naming conventions, expected inputs, and safe execution patterns.
Then open the reference appendix for the complete command inventory and exact syntax.
These aliases reduce repeated typing and preserve consistent workflow behavior on macOS, Linux, and WSL.

## When to use

Use this page before daily shell work when you need predictable command patterns.

## What is included

- Aliases: 0
- Helper functions: 2
- Source files: 1

## Quick examples

No alias examples are available for this category.

## Next step

Open the full list:

- [Update Reference Appendix](./reference.md)
