---
title: Clear Aliases | Dotfiles
description: Discover screen clear aliases in Dotfiles v0.2.495. Quick terminal reset and scrollback clearing commands for Zsh and Bash on macOS, Linux, and WSL.
lang: en-GB
author: Sebastien Rousseau
lastUpdated: true
image: /logo.png
imageAlt: Clear Aliases | Dotfiles preview
canonical: /en/aliases/clear/
robots: index, follow
draft: false
noindex: false
sitemap: true
breadcrumbTitle: Clear Aliases
pageType: docs
schemaType: WebPage
permalink: /en/aliases/clear/
---

# Clear Aliases

Short screen clear commands.

This page introduces one alias group and its common command shortcuts.
Read it first to understand naming conventions, expected inputs, and safe execution patterns.
Then open the reference appendix for the complete command inventory and exact syntax.
These aliases reduce repeated typing and preserve consistent workflow behavior on macOS, Linux, and WSL.

## When to use

Use this page before daily shell work when you need predictable command patterns.

## What is included

- Aliases: 8
- Helper functions: 0
- Source files: 1

## Quick examples

- `c` → `clear_screen`
- `cdw` → `cd_workspace`
- `clh` → `clear_history`
- `clp` → `print_working_dir`
- `cls` → `clear_list_current`
- `clt` → `clear_print_tree`
- `cpl` → `clear_pwd_list`
- `cplt` → `clear_pwd_tree`

## Next step

Open the full list:

- [Clear Reference Appendix](./reference.md)
