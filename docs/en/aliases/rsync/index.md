---
title: Rsync Aliases | Dotfiles
description: Explore rsync aliases in Dotfiles v0.2.495. Concise file synchronization shortcuts with progress, compression, and incremental support on any platform.
lang: en-GB
author: Sebastien Rousseau
lastUpdated: true
image: /logo.png
imageAlt: Rsync Aliases | Dotfiles preview
canonical: /en/aliases/rsync/
robots: index, follow
draft: false
noindex: false
sitemap: true
breadcrumbTitle: Rsync Aliases
pageType: docs
schemaType: WebPage
permalink: /en/aliases/rsync/
---

# Rsync Aliases

Short sync commands.

This page introduces one alias group and its common command shortcuts.
Read it first to understand naming conventions, expected inputs, and safe execution patterns.
Then open the reference appendix for the complete command inventory and exact syntax.
These aliases reduce repeated typing and preserve consistent workflow behavior on macOS, Linux, and WSL.

## When to use

Use this page before daily shell work when you need predictable command patterns.

## What is included

- Aliases: 2
- Helper functions: 0
- Source files: 1

## Quick examples

- `rs` → `rsync -avz`
- `rsync` → `rs`

## Next step

Open the full list:

- [Rsync Reference Appendix](./reference.md)
