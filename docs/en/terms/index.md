---
title: Terms of Use | Dotfiles
description: Terms of use for Dotfiles documentation. Licensing, copyright, acceptable use policy, and content restrictions for this open-source project.
lang: en-GB
author: Sebastien Rousseau
lastUpdated: true
image: /logo.png
imageAlt: Dotfiles terms of use
canonical: /en/terms/
robots: index, follow
draft: false
noindex: false
sitemap: true
breadcrumbTitle: Terms
pageType: legal
schemaType: WebPage
date: 2026-03-08
category: Legal
tags:
  - terms
  - legal
---

# Terms of Use

By using this site, you agree to the following terms.

## Content license

- Documentation text and code snippets are provided under the repository license unless otherwise stated.
- Third-party trademarks and product names belong to their respective owners.

## No warranty

The documentation is provided "as is" without warranty of any kind.

## Liability

The maintainers are not liable for indirect or consequential damages resulting from use of this content.

## Contact

Questions about these terms can be sent to [info@dotfiles.io](mailto:info@dotfiles.io).
