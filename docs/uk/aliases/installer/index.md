---
title: Аліаси інсталятора | Dotfiles
description: Короткі команди встановлення. Огляд для Dotfiles v0.2.495.
lang: uk-UA
author: Sebastien Rousseau
lastUpdated: true
image: /logo.png
imageAlt: Аліаси інсталятора | Dotfiles preview
canonical: /uk/aliases/installer/
robots: index, follow
draft: false
noindex: false
sitemap: true
breadcrumbTitle: Аліаси інсталятора
pageType: docs
schemaType: WebPage
permalink: /uk/aliases/installer/
---

# Аліаси інсталятора

Короткі команди встановлення.

Ця сторінка знайомить з однією групою аліасів та її поширеними скороченнями команд.
Прочитайте її спочатку, щоб зрозуміти угоди про іменування, очікувані вхідні дані та безпечні шаблони виконання.
Потім відкрийте довідковий додаток для повного переліку команд та точного синтаксису.
Ці аліаси зменшують повторний набір та зберігають послідовну поведінку робочого процесу на macOS, Linux та WSL.

## Коли використовувати

Використовуйте цю сторінку перед щоденною роботою в оболонці, коли вам потрібні передбачувані шаблони команд.

## Що включено

- Аліаси: 3
- Допоміжні функції: 0
- Файли джерел: 1

## Швидкі приклади

- `dot-install` → `bash $HOME/.dotfiles/install.sh`
- `dot-teleport` → `telegram`
- `telegram` → `bash $HOME/.dotfiles/scripts/ops/teleport.sh`

## Наступний крок

Відкрити повний перелік:

