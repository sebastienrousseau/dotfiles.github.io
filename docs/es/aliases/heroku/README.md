...
lang: es-ES
metaTitle: The Heroku aliases - Dotfiles (ES)
...
meta:
  ...
  - name: twitter:description
    content: The Heroku aliases, provide powerful commands that create shortcuts to frequently used Heroku commands. All of these functions and aliases are wrappers around the Heroku CLI which is the basic, fundamental tools of a Heroku system.
  - name: twitter:title
    content: The Heroku aliases - Dotfiles (ES)
  - name: og:title
    content: The Heroku aliases - Dotfiles (ES)
  - name: og:description
    content: The Heroku aliases, provide powerful commands that create shortcuts to frequently used Heroku commands. All of these functions and aliases are wrappers around the Heroku CLI which is the basic, fundamental tools of a Heroku system.
...
| hka   | `heroku access:add`    | Añadir new users to your app.                     |
| hkau  | `heroku access:update` | Actualizar existing collaborators on an team app.  |
| hkh   | `heroku help`          | Display help for heroku.                       |
| hkj   | `heroku join`          | Añadir yourself to a team app.                    |
...
| hkq   | `heroku leave`         | Eliminar yourself from a team app.               |
| hkr   | `heroku access:remove` | Eliminar users from a team app.                  |
...
| hku   | `heroku update`        | Actualizar the heroku CLI.                         |
...
| -------- | ------------------------- | ---------------------------------------------------------- |
| hkada    | `heroku addons:attach`    | Attach an existing Añadir-on resource to an app.              |
| hkadc    | `heroku addons:create`    | Crear a new Añadir-on resource.                              |
| hkadd    | `heroku addons:detach`    | Detach an existing Añadir-on resource from an app.            |
| hkaddoc  | `heroku addons:docs`      | Abrir an Añadir-on's Dev Center documentation in your browser. |
| hkaddown | `heroku addons:downgrade` | Change Añadir-on plan.                                        |
| hkadi    | `heroku addons:info`      | Mostrar detailed Añadir-on resource and attachment information.  |
| hkadk    | `heroku addons:destroy`   | Permanently destroy an Añadir-on resource.                    |
| hkadl    | `heroku addons`           | Lists your Añadir-ons and attachments.                        |
| hkado    | `heroku addons:open`      | Abrir an Añadir-on's dashboard in your browser.                |
| hkadp    | `heroku addons:plans`     | Listar all available plans for an Añadir-on services.           |
| hkadr    | `heroku addons:rename`    | Renombrar an Añadir-on.                                          |
| hkads    | `heroku addons:services`  | Listar all available Añadir-on services.                        |
| hkadu    | `heroku addons:upgrade`   | Change Añadir-on plan.                                        |
| hkadw    | `heroku addons:wait`      | Mostrar provisioning status of the Añadir-ons on the app.        |
...
| hkapj     | `heroku apps:join`             | Añadir yourself to a team app.                    |
...
| hkapq     | `heroku apps:leave`            | Eliminar yourself from a team app.               |
| hkapr     | `heroku apps:rename`           | Renombrar an app.                                 |
...
| hk2fas   | `heroku auth:2fa`         | Comprobar 2fa status.                                           |
...
| Alias  | Command                        | Description                           |
| ------ | ------------------------------ | ------------------------------------- |
| hkauc  | `heroku authorizations:create` | Crear a new OAuth authorization.     |
...
| hkbpad | `heroku buildpacks:add`      | Añadir new app Construir-pack, inserting into Listar of Construir-packs if necessary. |
...
| hkbpr  | `heroku buildpacks:remove`   | Eliminar a Construir-pack set on the app.                                      |
| hkbps  | `heroku buildpacks:search`   | Buscar for Construir-packs.                                                  |
...
| hkcad | `heroku certs:add`          | Añadir an SSL certificate to an app.                    |
...
| hkcr  | `heroku certs:remove`       | Eliminar an SSL certificate from an app.               |
| hkcu  | `heroku certs:update`       | Actualizar an SSL certificate on an app.                 |
...
| hkcid  | `heroku ci:debug`            | Opens an interactive test debugging session with the contents of the current directorio.        |
...
| hkcir  | `heroku ci:run`              | Run tests against current directorio.                                                           |
| hkcir2 | `heroku ci:rerun`            | Rerun tests against current directorio.                                                         |

## Heroku config aliases

| Alias | Comando               | Descripción                                                                |
| ----- | ------------------------ | -------------------------------- |
| hkclc | `heroku clients:create`  | Crear a new OAuth client.       |
| hkcli | `heroku clients:info`    | Mostrar details of an oauth client. |
| hkclk | `heroku clients:destroy` | Borrar client by ID.             |
| hkcll | `heroku clients`         | Listar your OAuth clients.         |
| hkcls | `heroku clients:rotate`  | Rotate OAuth client secret.      |
| hkclu | `heroku clients:update`  | Actualizar OAuth client.             |
...
| hkctrm      | `heroku container:rm`      | Eliminar the process type from your app.                       |
...
| hkdoa | `heroku domains:add`    | Añadir a domain to an app.                                       |
| hkdoc | `heroku domains:clear`  | Eliminar all domains from an app.                               |
| hkdoi | `heroku domains:info`   | Mostrar detailed information for a domain on an app.             |
| hkdor | `heroku domains:remove` | Eliminar a domain from an app.                                  |
| hkdou | `heroku domains:update` | Actualizar a domain to use a different SSL certificate on an app. |
...
| Alias  | Command               | Description                     |
| ------ | --------------------- | ------------------------------- |
| hkdyk  | `heroku dyno:kill`    | Detener app dyno.                  |
| hkdyrz | `heroku dyno:resize`  | Gestionar dyno sizes.              |
| hkdyrs | `heroku dyno:restart` | Reiniciar app dynos.              |
| hkdysc | `heroku dyno:scale`   | Scale dyno quantity up or down. |
| hkdyst | `heroku dyno:stop`    | Detener app dyno.                  |
...
| Alias  | Command             | Description                                                                    |
| ------ | ------------------- | ------------------------------------------------------------------------------ |
| hkgitc | `heroku git:clone`  | Clones a heroku app to your local machine at directorio (defaults to app name). |
...
| hkk:a  | `heroku keys:add`    | Añadir an SSH key for a user.            |
| hkk:cl | `heroku keys:clear`  | Eliminar all SSH keys for current user. |
| hkk:r  | `heroku keys:remove` | Eliminar an SSH key from the user.      |
...
| hkpgbkdl    | `heroku pg:backups:delete`                      | Borrar a backup.                                                                                                      |
...
| hkpgbkk     | `heroku pg:backups:unschedule`                  | Detener daily backups.                                                                                                   |
...
| hkpgcpa     | `heroku pg:connection-pooling:attach`           | Añadir an attachment to a database using connection pooling.                                                             |
| hkpgcr      | `heroku pg:credentials`                         | Mostrar information on credentials in the database.                                                                      |
| hkpgcrc     | `heroku pg:credentials:create`                  | Crear credential within database.                                                                                    |
...
| hkpglnkc    | `heroku pg:links:create`                        | Crear a link between data stores.                                                                                    |
...
| hkpgreset   | `heroku pg:reset`                               | Borrar all data in DATABASE.                                                                                          |
...
| hkpguf      | `heroku pg:unfollow`                            | Detener a replica from following and make it a writeable database.                                                       |
...
| hkpipea    | `heroku pipelines:add`      | Añadir this app to a pipeline.                                                                                                           |
| hkpipec    | `heroku pipelines:create`   | Crear a new pipeline.                                                                                                                |
...
| hkpiper    | `heroku pipelines:remove`   | Eliminar this app from its pipeline.                                                                                                    |
| hkpipern   | `heroku pipelines:rename`   | Renombrar a pipeline.                                                                                                                    |
| hkpipes    | `heroku pipelines:setup`    | Bootstrap a new pipeline with common settings and Crear a production and staging app (requires a fully formed app.json in the repo). |
| hkpipett   | `heroku pipelines:transfer` | Transfer ownership of a pipeline.                                                                                                     |
| hkpipeu    | `heroku pipelines:update`   | Actualizar the app's stage in a pipeline.                                                                                                 |
...
| hkplugsu  | `heroku plugins:update`    | Actualizar installed plugins.                    |
...
| hkpsc   | `heroku ps:copy`              | Copiar a archivo from a dyno to the local filesystem.                 |
| hkpse   | `heroku ps:exec`              | Crear an SSH session to a dyno.                                 |
| hkpsf   | `heroku ps:forward`           | Forward traffic on a local port to a dyno.                       |
| hkpsk   | `heroku ps:kill`              | Detener app dyno.                                                   |
| hkpsr   | `heroku ps:restart`           | Reiniciar app dynos.                                               |
| hkpsrs  | `heroku ps:resize`            | Gestionar dyno sizes.                                               |
| hkpss   | `heroku ps:stop`              | Detener app dyno.                                                   |
| hkpssc  | `heroku ps:scale`             | Scale dyno quantity up or down.                                  |
| hkpssck | `heroku ps:socks`             | Launch a SOCKS proxy into a dyno.                                |
| hkpst   | `heroku ps:type`              | Gestionar dyno sizes.                                               |
...
| hkredmt  | `heroku redis:maintenance`            | Gestionar maintenance windows.                                                          |
...
| hksessionsd | `heroku sessions:destroy` | Borrar (logout) OAuth session by ID. |
...
| hkspc    | `heroku spaces:create`           | Crear a new space.                                                 |
...
| hkspvc   | `heroku spaces:vpn:connect`      | Crear VPN.                                                         |
...
| hkspvu   | `heroku spaces:vpn:update`       | Actualizar VPN.                                                         |
...
| hkwha   | `heroku webhooks:add`             | Añadir a webhook to an app.            |
